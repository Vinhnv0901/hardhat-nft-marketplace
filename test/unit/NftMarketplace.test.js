  ...
  : describe("Nft Marketplace Unit Tests", function () {
      ...
      describe("cancelListing", function () {
        ...
        it("reverts if anyone but the owner tries to call", async function () {
          await nftMarketplace.listItem(basicNft.target, TOKEN_ID, PRICE); ///market đang nắm quyền token_id này
          nftMarketplace = nftMarketplaceContract.connect(user); /////////////user đang kết nối với market
          await basicNft.approve(user.address, TOKEN_ID); ////////////////////token_id này lại cho quyền có user
          await expect(
            nftMarketplace.cancelListing(basicNft.target, TOKEN_ID) //////////user đang gọi hàm này
          ).to.be.revertedWith("NotOwner");
        });
